...
export async function POST(req: NextRequest) {
  try {
    ...
    const recipientPublicKeyHex = body.recipientPublicKeyHex?.trim();
    ...
    if (!recipientPublicKeyHex) {
      return NextResponse.json({ error: "recipientPublicKeyHex is required" }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to build coupon payout transfer" },
      { status: 500 }
    );
  }
}
